...
This piece compares two ways of building on a blockchain. Polkadot's Substrate framework hands most of the work to the chain itself: you describe the rules, the chain executes them. CKB works the other way around: it hands the work to the developer, and just checks that what you built follows the rules. If you've ever wondered why CKB feels more hands-on than other chains, that difference is the reason, and it's the subject of everything below.

---

The first thing CKB asks you to do is construct a transaction by hand.

Not submit an extrinsic (Polkadot's term for a request that changes on-chain state). Not call a contract method. Build the inputs yourself, find the cells you want to consume, decide which outputs to create, figure out the capacity, encode the data, attach the right scripts, then sign and broadcast. The chain doesn't do any of that for you. It receives your proposed state transition and either accepts or rejects it.

If you're coming from Polkadot, this feels backwards. On Substrate, constructing state transitions is the runtime's job. You write a pallet (a self-contained module of chain logic, like a plugin) that defines valid transitions; FRAME, Substrate's framework for assembling pallets, wires it into the block execution pipeline; PAPI gives your frontend a fully typed API to call into it. The division of labor is clear: developers define rules, the chain enforces and executes them. You never manually assemble a storage write, you simply call a transfer function and the runtime handles everything underneath.
...
**The runtime is a single WebAssembly (WASM) binary.** Every pallet in your chain compiles into one WASM blob. That blob is the law: it defines every valid state transition, every callable function, every storage layout. Nodes execute that blob to agree on state. When you deploy a parachain, you're deploying a runtime. When you upgrade, you replace the runtime with a new WASM binary through an on-chain governance vote. The upgrade is **forkless**, no coordinated hard fork, no nodes going offline, because the upgrade itself is just another state transition that the old runtime approves.
...
**The relay chain absorbs your consensus.** As a parachain developer, you don't implement consensus. You write a runtime; Polkadot's collator and validator machinery handles block production and finality. Your job ends at "define valid state transitions." Everything else is infrastructure.

These three things together, a single runtime, owned storage, pooled consensus, define the Polkadot developer experience. They're so fundamental that most Substrate developers wouldn't list them as assumptions. They're just "how blockchains work."
...
_If you write pallets or runtime logic, this section is for you._

### There Is No Runtime

CKB has no runtime in the Substrate sense. There is no WASM binary that every node executes to determine valid state transitions. There is no single authority that owns storage. There is no place to put your storage and call definitions.

What CKB has instead is the **cell model**.

A cell is the atomic unit of state on CKB. It has four parts:
...
- **Lock script**: a program that controls who can spend this cell, the ownership rule
- **Type script**: an optional program that governs what the cell's data means, the logic rule
...
In Substrate, a pallet is a Rust module that directly reaches into its own storage and mutates it. It owns that storage. The mutation is the point.

Now, what about those lock script and type script fields on a cell? Each one points to a small compiled program called a **script**, written to run on RISC-V, a general-purpose processor architecture. When a transaction is submitted, the CKB-VM runs the scripts referenced by every consumed and created cell, and each one gets to vote on whether the transaction is valid.

On CKB, a script doesn't mutate anything. It's a **validator**. It receives a complete picture of the proposed transaction, all inputs, all outputs, all cell data, and returns either "valid, let this through" or an error code that rejects the transaction. It has no ability to reach out and change something. The state change is proposed by the transaction constructor (your wallet, your dapp, your SDK), and the scripts on the consumed and created cells decide whether to allow it.

This is the fundamental shift: from **mutating programs** to **validating programs**.

A simple token transfer validation, written as a CKB type script, works like this in plain terms: it adds up the token amounts across all the input cells that carry this type script, adds up the token amounts across all the output cells that carry it, and checks that the input total is at least as large as the output total, so no tokens can be created out of thin air. If the totals don't add up, the transaction is rejected; if they do, it's approved.
...
The binary is deployed on-chain as a cell, just data in a cell, no different in structure from any other cell. Other transactions reference it via what's called a cell dependency, which is how the CKB-VM knows where to load the script from at validation time.

This is "bring your own cryptography and consensus primitives" taken to its logical conclusion. There are no hardcoded shortcuts baked into the protocol. There's no privileged pallet registry. Every system script, including the default lock that CKB addresses use, is just a cell on-chain that anyone can reference. You can write a competing implementation, deploy it, and it works today. No governance vote. No hard fork.
...
If you're used to writing Ethereum-style smart contracts (including on Polkadot, via `pallet-revive`, its Ethereum-compatible option), the shift is even sharper than for pallet writers. A Solidity contract keeps its own permanent storage, fires events other code can listen for, and knows who is calling it. None of that maps to CKB. What you'd write instead is a type script: a small program that looks at a proposed transaction and simply approves or rejects it. "Your contract's balance" becomes "however many tokens are sitting in cells that reference your type script, right now, in this transaction." There's no persistent state for an attacker to manipulate through repeated calls, a common category of smart-contract bug, because there's no ongoing state to call into in the first place.

---

## Part II: For Dapp Developers

_If you build frontends, SDKs, indexers, or off-chain tooling, this section is for you._

### There Are No Contract Methods

The Polkadot dapp developer's mental model is service-oriented. The modern standard is PAPI (Polkadot-API), a light-client-first TypeScript library whose types are generated directly from on-chain metadata. Your editor immediately knows every storage key, every extrinsic, every event on the chain. Querying a balance or submitting a transfer looks like calling a fully typed method on a service, the way you'd call any typed API in a modern app.

The key thing PAPI gives you: the chain's entire surface area is statically typed from metadata. Your IDE knows what a query will return before you run a single line of code. The chain is a typed service you call.
...
On CKB, none of this exists. There is no typed transfer method to call. There is no metadata. There is no schema. The dapp developer's job changes from **calling a typed service** to **constructing a state transition from raw bytes**.

Here's what a token transfer looks like from the dapp side on CKB, in plain steps:

1. **Collect input cells**: scan the live cell set for cells owned by the sender whose type script matches the token you want to send
...
4. **Attach cell dependencies**: include a reference to the xUDT script cell so the CKB-VM can load the validation logic
5. **Sign and submit**: sign the transaction with the sender's key (satisfying the lock scripts on the input cells) and broadcast it
...
Modern SDKs like CCC (CKBers' Codebase) provide helpers that pick input cells and calculate fees for you, handling the tedious parts. But you still declare the outputs by hand. The transaction, not a method call, remains the developer's unit of thought.
...
On Polkadot, querying state means reading from a well-defined storage layout that the runtime exposes, and PAPI makes this frictionless because it generates TypeScript types directly from that metadata. The chain tells PAPI its schema; PAPI tells your IDE. You never manually decode storage keys or guess at data layouts. When the runtime upgrades and storage changes, your descriptors update automatically, and the type system catches breakage at compile time.

On CKB, there is no metadata. There is no schema the chain publishes. There is no equivalent one-command setup. A CKB node lets you query the live set of cells directly via RPC, filtered by lock script, by type script, or by capacity range. But the meaning of the bytes in each cell's data field is entirely up to you to know and decode. A typical question looks like: "give me all unspent cells whose lock script matches this address and whose type script matches this token type."

The result is a list of live cells. You sum the amounts in their data fields to get the balance. There is no typed query that returns a single number. There is no metadata telling you where in the cell the balance is stored. You have to know that, because you wrote the type script, or because you read the token standard's specification. Balance is a derived quantity computed from raw bytes, exactly like Bitcoin's UTXO model, and exactly unlike PAPI's compile-time-typed storage reads.
...
On Polkadot, access control to state is mostly handled by the runtime's origin system. A dispatchable call checks that the caller signed the request, and then has full permission to mutate whatever storage it references, as long as the runtime's logic allows it.

On CKB, ownership is enforced by the lock script on every cell, evaluated independently by every full node in the network. There's no runtime with privileged access. There's no "the contract allows this, so it's fine." If a cell's lock script says a transaction requires a certain signature, then no transaction consuming that cell will be valid without it, not because nodes are well-behaved, but because every node runs the script independently and rejects anything that fails.

This makes ownership composable in ways that Polkadot's model isn't. You can put a cell under a multisig lock script that requires keys from two different parties, with no central contract coordinating. You can put a cell under a time-lock that opens after a certain block number. These are all just RISC-V programs. Any code that compiles to RISC-V and returns a pass or fail result is a valid lock script.
...
**Cryptography is not privileged.** CKB's native address format uses the same cryptographic curve as Bitcoin and Ethereum. But it's not hardcoded at the protocol level, it's implemented as a lock script in a system cell. JoyID uses a completely different lock script based on device passkey signatures instead. Both are first-class citizens. There are no built-in shortcuts, no hard forks required, no permission needed.
...
**Ownership as a first-class primitive.** On Polkadot, access control is mostly runtime-mediated: prove you signed the request and you're in. On CKB, ownership is the lock script on the cell, evaluated independently by every node. Multisig, time-locks, passkey-based auth (JoyID), or entirely custom access rules ship without touching a runtime or waiting on a governance upgrade. Polkadot doesn't really have an equivalent design surface for this.

**A directly-inspectable execution model.** On CKB, a transaction body enumerates every cell it consumes and produces. No hidden side effects, no block-level runtime hooks mutating state you didn't ask for. You still have to audit the scripts themselves, but the state footprint of any transaction, which cells changed, and how, is fully visible in the transaction body. Substrate and PAPI are clean, but this explicit-state-footprint mental model pays off for audits, dapp security reviews, and protocol analysis.
